    ...
    return (
        <div id='ourContact' className='w-full relative flex flex-col items-center justify-start'>
            ...
            <section className='absolute text-center py-12'>
                <h1 className='text-[#625EF6] font-bold text-2xl'>GET IN TOUCH</h1>
            ...
            <section className='absolute top-44'>
                {contactDetails && contactDetails.map((item, index) => (
                    <div key={index} className='my-4'>
                        <div className='bg-white opacity-90 rounded-full h-16 max-w-[540px]'>
                            <div className='flex items-center h-full px-2'>
                                {item.icon && <img className='bg-[#625EF6] p-2 rounded-full w-12 h-12'
                                    src={item.icon}
                                />}

                                {!item.icon && <span className='ml-12'></span>}

                                <p className='font-bold flex items-center h-full px-8'>
                                    {item.label}
                                </p>
                            </div>
                        </div>
